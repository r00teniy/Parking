...
{
    //SetPlotNumberAttribute in ParkingBlocks
    internal static string SetPlotNumbersInBlocks(List<PlotBorderModel> plots, string xRefName, bool everywhere)
    {
        ...
        using (Transaction tr = db.TransactionManager.StartTransaction())
        {
                    ...
                    foreach (ObjectId objectId in btr)
                    {
                        if (objectId.ObjectClass == Variables.rxClassBlockReference)
                        {
                            ...
                            if (br.AttributeCollection.Count == 4)
                            {
                                ...
                                var attRef = tr.GetObject(oi3, OpenMode.ForRead) as AttributeReference;
                                if (attRef.Tag == "НОМЕР_УЧАСТКА")
                                {
                                    attRef.UpgradeOpen();
                                    var wasSet = 0;
                                    foreach (var plot in plots)
                                    {
                                        if (DataImport.CheckIfObjectIsInsidePolyline(plot.Polyline, br) != Autodesk.AutoCAD.BoundaryRepresentation.PointContainment.Outside)
                                        {
                                            attRef.TextString = plot.PlotNumber;
                                            wasSet = 1;
                                            break;
                                        }
                                    }
            ...
            if (numberOfErrors != 0)
            {
                System.Windows.MessageBox.Show($"{numberOfErrors} блоков парковок находятся вне участков", "Error");
            }
            tr.Commit();
        }
        return output;
